...
object VcfUtilTests {
  ...
  def runTests(infile : String, outfile : String){
    ...
    for(v <- wrappedVcfIterator){
      if(v.getAlternateAlleles.length > 1){
        reportln("Alt Alleles: "+v.getAlternateAlleles.length,"debug");
      }
        ...
        val valueList = infoKeyList.zip(severityLevels).map{
          case (infoKey: String, lvls : List[String]) => {
            if(v.hasAttribute(infoKey)){
              val valueList = v.getAttributeAsList(infoKey)
              valueList.foldLeft("NoVal")((soFar,v)=> {
                lvls.get( math.max(lvls.indexOf(soFar),lvls.indexOf(v)) );
              }).toString();
            } else {
              "NoVal";
            }
